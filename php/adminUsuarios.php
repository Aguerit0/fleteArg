...
//EXTRAIGO IDS DEL BOTON ELIMINAR
if (isset($_GET['eliminar'])) {
  $ids = explode('_', $_GET['eliminar']);
  $idUsuario = $conexion->real_escape_string($ids[0]);
  $idEstado = $conexion->real_escape_string($ids[1]);

  // Consulta para eliminar el usuario utilizando ambos IDs
  $sqlEliminarUsuario = "UPDATE usuario SET eliminado = 1 WHERE idUsuario = $idUsuario";
  //extraigo idCliente de idUsuario
  $sqlIdCliente = "SELECT idCliente FROM usuario WHERE idUsuario = $idUsuario";
  $resIdCliente = mysqli_query($conexion, $sqlIdCliente);
  if($re = $resIdCliente->fetch_assoc()){
    $idClienteElim = $re['idCliente'];
  }
  //eliminar cliente vinculado al usuario
  $sqlEliminarCliente = "UPDATE cliente SET eliminadoCliente = 1 WHERE idCliente = $idClienteElim";
  if ($conexion->query($sqlEliminarUsuario) && $conexion->query($sqlEliminarCliente)) {
      // La eliminación se realizó con éxito
      ?>
      <script>
        if(confirm("Usuario eliminado con éxito!!")) {
